...
\textbf{Implementation languages:} C\texttt{++} (low-level kernels),
Python (high-level demos), MATLAB (verification).\\[4pt]
...
\ff{Applications}
  ...
  \pri{Recommended}.
\ff{Languages} Python (construct/visualize vectors), C\texttt{++} \pri{Enrichment}.
\ff{Textbook refs}
...
\ff{Applications}
  ...
  \pri{Enrichment}.
\ff{Languages}
  C\texttt{++} kernels (inner product, cosine similarity, with compiler-flag
  optimization) \pri{Core}; Python demos (KNN, $k$-means) \pri{Recommended};
  MATLAB verification.
\ff{Textbook refs}
...
\ff{Languages} Python (complex plane visualization); MATLAB.
\ff{Textbook refs}
...
\ff{Languages}
  C\texttt{++} (matmul, mat--vec with \texttt{-O2/-O3}, cache blocking)
  \pri{Core}; Python/MATLAB verification.
\ff{Textbook refs}
...
\ff{Languages}
  C\texttt{++} (float vs.\ double behavior); Python/MATLAB (epsilon,
...
\ff{Languages}
  C\texttt{++} (elimination kernel) \pri{Recommended};
  MATLAB (\texttt{lu}, verification) \pri{Core}.
\ff{Textbook refs}
...
\ff{Languages} Python (visualize $2$-D/$3$-D transforms); MATLAB.
\ff{Textbook refs}
...
\ff{Languages} Python/MATLAB (rank, null-space bases).
\ff{Textbook refs}
...
\ff{Languages}
  Python (linear regression demo) \pri{Core};
  MATLAB (\texttt{qr}, \texttt{chol}, verification);
  C\texttt{++} (QR kernel) \pri{Enrichment}.
\ff{Textbook refs}
...
\ff{Languages}
  Python (PCA, SVD, Eigenfaces, spectral graph demos) \pri{Core};
  MATLAB (\texttt{eig}, \texttt{svd}, verification);
  C\texttt{++} (power iteration) \pri{Enrichment}.
\ff{Textbook refs}
...
\ff{Topics}
  ...
  \textbf{Levenberg--Marquardt}. Quasi-Newton \textbf{BFGS} (mention).
  Constrained optimization and the \textbf{support vector machine} under the
  Vapnik objective; kernel methods (reuse Unit~2 kernels). Stochastic /
  Monte-Carlo optimization (SGD) as an application \pri{Enrichment}.
  Low-rank matrix factorization with a \emph{masked} loss over observed entries
  (Alternating Least Squares / SGD) for collaborative filtering. Model-selection
  methodology: train/validation/test splits and cross-validation as estimates of
  generalization error (ties to Unit~6 tolerances) \pri{Recommended}.
\ff{Key theorems}
...
\ff{Tools / formulas}
  ...
  LM damped step $(J^\top J+\lambda I)\Delta=-J^\top r$; SVM margin/objective.
\ff{Algorithms}
  ...
  \textbf{BFGS} (mention) \pri{Enrichment};
  \textbf{SVM training} \pri{Recommended};
  \textbf{SGD / Monte-Carlo} \pri{Enrichment}.
\ff{Applications}
  Model fitting; classification via SVM; the full ML pipeline tying the course
  together (embeddings $\to$ similarity $\to$ optimization); collaborative-
  filtering recommender; signal trilateration (GPS-style positioning).
\ff{Languages}
  Python (GD, Newton, SVM demos) \pri{Core};
  MATLAB (\texttt{lsqnonlin}, verification) \pri{Recommended};
  C\texttt{++} (GD kernel) \pri{Enrichment}.
\ff{Textbook refs}
  \textbf{[BV]} Ch.~18--19 (nonlinear \& constrained least squares:
  Gauss--Newton, Levenberg--Marquardt), App.~C;
  gradient descent, Newton, BFGS, SVM \emph{supplementary}.
...
method, choose Gauss--Newton/LM for nonlinear least squares, and train an SVM,
articulating how each method's behavior depends on conditioning.
\end{gateway}

%==============================================================================
\section*{Assessment mapping (tentative)}
%==============================================================================
\begin{center}
\renewcommand{\arraystretch}{1.25}
\begin{tabular}{@{}l l l@{}}
...
\midrule
Project 1 [Assigned] & after 2  & \#1 K-means image color segmentation (Python / C\texttt{++}) \\
Project 2 [Assigned] & after 11 & \#5 Eigencats via PCA (Python) \\
Project 3 [Assigned] & after 12 & \#7 Mini-ML: OLS + SVM + KNN with validation (Python) \\
Project 4 [Assigned] & after 12 & \#8 Signal trilateration: Gauss--Newton / LM (Python / MATLAB) \\
...
%==============================================================================
\section*{Computational Projects \& Application Demos}
%==============================================================================
This is a \emph{menu}, not an assignment quota: every item below is documented,
but only a small subset is assigned to students as an implementation
(\textbf{[Assigned]}); the complement are instructor-led \textbf{[Demo]}s that
still put the application in front of the class. The list is open --- further
items may be appended as \#10, \#11, \dots\ without disturbing the ordering.
...
\subsection*{Assigned implementations}
\begin{description}[leftmargin=0pt,style=nextline,font=\normalfont\sffamily\bfseries\color{RoyalBlue},itemsep=4pt]
\item[\#1 -- K-means image color segmentation (after Unit 2)]
  ...
  Lloyd's iteration and its convergence. Python (NumPy/Pillow) or a C\texttt{++}
  kernel.
...
\item[\#8 -- Signal trilateration (after Unit 12)]
  ...
  conditioning. Python or MATLAB.
\end{description}

\subsection*{Lecture demos}
\begin{description}[leftmargin=0pt,style=nextline,font=\normalfont\sffamily\bfseries\color{RoyalBlue},itemsep=4pt]
