...
%
%  SCOPE.  This file currently covers exactly three lectures:
...
%  material -- none of that has been covered in class yet.  The vectors Part 2
%  lecture stops after S12.6 (the orthogonal piece); Cauchy-Schwarz opens S13 in
%  Part 3 and has NOT been lectured -- see the note on Problem E.2 below.
%
%  MORE PROBLEMS TO COME.  Parts are lettered (A, B, C, D, E, F) rather than
...
%  anything already written.  Add them as new \section blocks before Part F, or
%  extend Part F's Section F.x list, whichever fits.  Part E (dot product,
%  cosine similarity, projection) was added this way; adding it renumbered the
%  NumPy part from E to F and nothing else.
%
%  DELIBERATE OMISSIONS, so that nothing here outruns the lectures:
%   - Parts C and D still avoid the dot product, because they were written
%     against the complex lecture alone: the complex lecture's S7 proof that
%     multiplication by i is a rotation *does* use it, so Problem C.7 and D.5
%     stay routed through the modulus, and the cross-reference is flagged
%     honestly in the text.  Part E is where the dot product is exercised.
%   - No metrics, no S^1 notation.  Part E uses normalisation (\hat{u}) only
%     implicitly, through comp and proj; it never asks for a unit vector.
%   - No argument/polar form beyond cos(theta) + i sin(theta) and Euler.
%   - Part E.2 (Cauchy-Schwarz) and E.5 (the triangle inequality by Cauchy-
%     Schwarz) are S13 material, from vectors Part 3, which has not been
%     lectured.  Both are fully scaffolded so they are self-contained from S10-
%     S12, and the section preamble says so out loud.
%
%  NO ANSWER KEY, by design.  Part F is the answer key: every arithmetic problem
%  in Parts A, C and E can be checked by the student in NumPy.  That is the whole
...
Vector Unit, Part 2 & \S10--\S12 &
  relative orientation --- parallel, antiparallel, orthogonal; the angle
  between two vectors; the dot product and its laws \textbf{D1}--\textbf{D4};
  cosine similarity; the orthogonality test; projection, the signed component,
  and the orthogonal piece \\[3pt]
...
\begin{itemize}[leftmargin=1.4em,itemsep=7pt,topsep=3pt]
  \item \textbf{Parts A--E} --- your written work, as a single PDF named
        ...
        \textbf{Type Parts A--E up instead and you earn $+5$ points of extra
        ...
        mathematics --- and the extra credit is for the whole of Parts A--E, not
        for typing up some of it. The file you submit must still be a PDF.

  \item \textbf{Part F} --- two Python files, \texttt{hw1.py} (your work, one
        ...
        F is graded from what it prints. \emph{How to structure your submission},
        in Part F, specifies both files in full.

        \textbf{Python is the only language accepted for the programming.} Any
        other language earns no credit for Part F, however correct it is.
\end{itemize}
...
\textbf{Parts A--E.} Label each answer with the number of the problem it answers
...
\textbf{Part F.} The labelling is built into the file structure specified under
...
Part F is graded by running \texttt{runhw1.py} and reading what it prints --- on
whether it produces correct answers, not on programming style.

\medskip
\noindent\textbf{Use Part F to check Parts A, C and E.} Every numerical answer in
those parts can be verified in two lines of NumPy, and you are encouraged to
...
partial credit at best. And no proof --- in Part B, Part D, or the proof problems
at the end of Part E --- can be checked this way, since code confirms a statement
only for the numbers you tried, not for all of them.

\medskip
\noindent\textbf{On the proof problems (Parts B and D, and E.2, E.5, E.6).}
These are short, and none of them requires a clever idea. What they require is that you write down
...
\begin{enumerate}[label=\textbf{(\alph*)},leftmargin=2.6em,itemsep=7pt,topsep=6pt]
  ...
  \item Write each of the four in trigonometric form $\cos\theta+i\sin\theta$,
        ...
        use \texttt{np.angle} from Part F) and say which quadrant it is in.
...
%==============================================================================
\section{Dot product, cosine similarity, and projection}
%==============================================================================
\emph{Vector Unit, Part 2.} Throughout this part, $\vc{u}=[\,u_1,\,u_2\,]$ and
$\vc{v}=[\,v_1,\,v_2\,]$ denote vectors in $\R^{2}$, and
\[
  \ip{\vc{u}}{\vc{v}} \;=\; \vc{u}\cdot\vc{v} \;=\; u_1v_1+u_2v_2
\]
is their dot product. Both notations mean the same number and you may use
either. Recall the three objects the lecture built on top of it: the
\textbf{cosine similarity} of two \emph{nonzero} vectors,
\[
  \cos(\vc{u},\vc{v}) \;=\; \frac{\ip{\vc{u}}{\vc{v}}}{\norm{\vc{u}}\,\norm{\vc{v}}}
  \;\in\;[-1,1] ,
\]
and, for $\vc{v}\neq\vc{0}$, the \textbf{component of $\vc{u}$ along $\vc{v}$}
and the \textbf{projection of $\vc{u}$ onto $\vc{v}$},
\[
...
\heads{The subscript names the vector you are projecting \emph{onto}. So
$\proj{\vc{v}}{\vc{u}}$ is $\vc{u}$'s shadow on $\vc{v}$, it is a \emph{vector},
and it points along $\vc{v}$ --- while $\comp{\vc{v}}{\vc{u}}$ is a
\emph{signed scalar}. Swapping the two arguments gives a different answer.}

\medskip
\noindent Leave dot products and components in exact form. A component such as
$-11/\sqrt{10}$ is a finished answer; so is $2\sqrt5$.

\medskip
\noindent \textbf{Two of these run slightly ahead of the lectures.} Problems
E.2 and E.5 are the Cauchy--Schwarz inequality and the triangle inequality
proved from it, which belong to \S13 --- the opening of vectors Part 3, which we
have not reached. They are here anyway because they are the natural payoff of
\S11, and both are scaffolded step by step so that everything you need is
\S10--\S12 plus the hints printed with the problem. Nothing in them requires a
fact you have not been given.

%------------------------------------------------------------------------------
\begin{prob}[The algebra of the dot product, by hand]
Let
\[
  \vc{u}=[\,3,\,-1\,],\qquad
  \vc{v}=[\,2,\,5\,],\qquad
...
\]
and let $\vc{0}=[\,0,\,0\,]$. Compute each quantity below. Where two quantities
are asked for on the same line, compute them \emph{separately} --- do not assume
in advance that they agree --- then say in a few words which law of \S11.8
(\textbf{D1} symmetry, \textbf{D2} additivity, \textbf{D3} homogeneity,
\textbf{D4} positive definiteness) the agreement illustrates.

\begin{enumerate}[label=\textbf{(\alph*)},leftmargin=2.6em,itemsep=7pt,topsep=6pt]
  \item $\ip{\vc{u}}{\vc{v}}$ \ and \ $\ip{\vc{v}}{\vc{u}}$.
  \item $\ip{\vc{u}}{\vc{v}+\vc{w}}$ \ and \
        $\ip{\vc{u}}{\vc{v}}+\ip{\vc{u}}{\vc{w}}$.
  \item $\ip{\alpha\vc{u}}{\vc{v}}$, \ $\alpha\,\ip{\vc{u}}{\vc{v}}$, \ and \
        $\ip{\vc{u}}{\alpha\vc{v}}$. All three are equal; which law, applied in
        which slot, is doing that?
  \item $\ip{\vc{u}}{\vc{u}}$ \ and \ $\norm{\vc{u}}^{2}$.
  \item $\ip{\vc{u}+\vc{v}}{\vc{u}+\vc{v}}$ \ and \
        $\norm{\vc{u}}^{2}+2\ip{\vc{u}}{\vc{v}}+\norm{\vc{v}}^{2}$.
  \item $\ip{\vc{u}}{\vc{0}}$. Then say, in one sentence, why
        $\ip{\vc{x}}{\vc{0}}=0$ for \emph{every} $\vc{x}\in\R^{2}$ --- and why
        this does \emph{not} contradict \textbf{D4}.
  \item $\ip{\alpha\vc{u}+\beta\vc{v}}{\vc{w}}$ \ and \
        $\alpha\,\ip{\vc{u}}{\vc{w}}+\beta\,\ip{\vc{v}}{\vc{w}}$.
  \item $\lvert\ip{\vc{u}}{\vc{v}}\rvert$ \ and \ $\norm{\vc{u}}\,\norm{\vc{v}}$,
        the second to three decimal places. Which is larger, and by how much?
\end{enumerate}

\noindent Finally, in one sentence: part (g) was not on the list \textbf{D1}--\textbf{D4}.
Which two of those laws, used together, give it?
\end{prob}

%------------------------------------------------------------------------------
\begin{prob}[The Cauchy--Schwarz inequality]
Prove that for \emph{all} $\vc{u},\vc{v}\in\R^{2}$,
\[
  \bigl\lvert\ip{\vc{u}}{\vc{v}}\bigr\rvert
  \;\le\;
  \norm{\vc{u}}\,\norm{\vc{v}} .
\]
Part (h) of Problem E.1 was one instance of this; here you must establish it for
every pair at once.

\hint{Let $t\in\R$ be arbitrary and expand $\norm{\vc{u}+t\vc{v}}^{2}$ using
\textbf{D1}--\textbf{D4}. By \textbf{D4} the result is a quadratic in $t$ that is
never negative, and one particular choice of $t$ turns that fact into the
inequality you want. Handle $\vc{v}=\vc{0}$ on a line of its own.}
\end{prob}

%------------------------------------------------------------------------------
\begin{prob}[Cosine similarity, and reading the arrangement off it]
For each pair below, compute $\ip{\vc{u}}{\vc{v}}$, $\norm{\vc{u}}$,
$\norm{\vc{v}}$ and the cosine similarity $\cos(\vc{u},\vc{v})$ in exact form.
Then give the angle $\theta$ between the two vectors in degrees, and state which
of \emph{parallel}, \emph{antiparallel}, \emph{orthogonal} or \emph{none of the
three} describes the pair.
...
\noindent Each of the four values $0$, $1$, $-1$ and $\tfrac{\sqrt2}{2}$ occurs
exactly once among your four answers; if one of yours is not one of those, you
have an arithmetic error to find.

\begin{enumerate}[label=\textbf{(\roman*)},leftmargin=2.6em,itemsep=7pt,topsep=6pt]
  \item For the pair you classified as parallel and the pair you classified as
        antiparallel, exhibit the scalar $\alpha$ with $\vc{v}=\alpha\vc{u}$,
        and say what the sign of $\alpha$ has to do with which of the two labels
        applies.
  \item Replace $\vc{v}$ in part (b) by $100\,\vc{v}$ and recompute
        $\cos(\vc{u},\vc{v})$. Explain in one sentence why the answer did not
        move, and what that says about \emph{what} cosine similarity measures.
\end{enumerate}
\end{prob}

%------------------------------------------------------------------------------
\begin{prob}[Projection and the signed component]
In each row below, $\vc{u}$ is the vector being projected and $\vc{v}$ is the
vector projected \emph{onto}. Note that $\vc{v}\neq\vc{0}$ in every case.

\begin{center}
\renewcommand{\arraystretch}{1.2}
\begin{tabular}{@{}c c c@{}}
\toprule
 & $\vc{u}$ (projected) & $\vc{v}$ (projected onto) \\
\midrule
(a) & $[\,4,\,3\,]$   & $[\,1,\,0\,]$   \\
(b) & $[\,3,\,4\,]$   & $[\,2,\,1\,]$   \\
(c) & $[\,-2,\,5\,]$  & $[\,3,\,-1\,]$  \\
...
\noindent For \emph{each} of the four rows, carry out all four steps.

\begin{enumerate}[label=\textbf{Step \arabic*.},leftmargin=4.8em,itemsep=6pt,topsep=6pt]
  \item Compute $\ip{\vc{v}}{\vc{u}}$ and $\norm{\vc{v}}^{2}$.
  \item State the \textbf{component} $\comp{\vc{v}}{\vc{u}}$ --- a scalar ---
        in exact form.
  \item State the \textbf{projection} $\proj{\vc{v}}{\vc{u}}$ --- a vector ---
        in bracket notation.
  \item Compute the orthogonal piece $\vc{u}-\proj{\vc{v}}{\vc{u}}$, and check
        that $\ip{\vc{v}}{\,\vc{u}-\proj{\vc{v}}{\vc{u}}}=0$.
\end{enumerate}
\end{prob}

%------------------------------------------------------------------------------
\begin{prob}[The triangle inequality, proved]
Part A.3 checked the triangle inequality on four numerical pairs and Problem B.5
found the case where it is an equality, but neither proved it. Prove it now: for
all $\vc{u},\vc{v}\in\R^{2}$,
\[
  \norm{\vc{u}+\vc{v}} \;\le\; \norm{\vc{u}}+\norm{\vc{v}} .
\]

\hint{Start by squaring the left-hand side. Use \textbf{D1}--\textbf{D4} to
expand
\[
  \norm{\vc{u}+\vc{v}}^{2}
  ...
  \;=\; \norm{\vc{u}}^{2}+2\ip{\vc{u}}{\vc{v}}+\norm{\vc{v}}^{2} .
\]
Now bound the middle term. You have $\ip{\vc{u}}{\vc{v}}\le\lvert\ip{\vc{u}}{\vc{v}}\rvert$
for free, and Cauchy--Schwarz (Problem E.2) bounds
$\lvert\ip{\vc{u}}{\vc{v}}\rvert$ by $\norm{\vc{u}}\,\norm{\vc{v}}$. Substitute,
recognise the right-hand side as a perfect square, and take square roots.}
\end{prob}

%------------------------------------------------------------------------------
\begin{prob}[The orthogonal piece is orthogonal]
Let $\vc{u},\vc{v}\in\R^{2}$ with $\vc{v}\neq\vc{0}$, and write
\[
  \vc{w} \;=\; \proj{\vc{v}}{\vc{u}} \;=\; \frac{\ip{\vc{v}}{\vc{u}}}{\norm{\vc{v}}^{2}}\,\vc{v} .
\]
Prove that $\vc{v}$ is orthogonal to $\vc{u}-\vc{w}$, that is, that
\[
  \ip{\vc{v}}{\,\vc{u}-\vc{w}} \;=\; 0 .
\]

\hint{Write $c=\ip{\vc{v}}{\vc{u}}/\norm{\vc{v}}^{2}$, so that $\vc{w}=c\,\vc{v}$,
and expand $\ip{\vc{v}}{\vc{u}-c\vc{v}}$ using \textbf{D2}, \textbf{D3} and
\textbf{D4}. The whole proof is three lines. Do \emph{not} write anything in
components: use only the four laws, so that the argument would survive being
moved to $\R^{n}$ unchanged.}
\end{prob}
...
to submit, then do Problems F.1--F.8. If you have never programmed before, that
is expected and fine: everything you need is below, and nothing here is harder
than the arithmetic you did by hand in Parts A, C and E.
...
\texttt{np.isclose} exists, and Problem F.6 asks you to think about it.}

\subsection{How to structure your submission}

Part F is submitted as \textbf{two} Python files with different jobs.

\begin{itemize}[leftmargin=1.4em,itemsep=6pt,topsep=4pt]
  \item \texttt{hw1.py} --- \textbf{your work}. One function per lettered part,
        named \texttt{q}\,\emph{number}\,\emph{letter}: Problem F.2(a) becomes
        \texttt{q2a}, Problem F.6(c) becomes \texttt{q6c}. A question with no
        lettered parts gets one function named for the question alone, so F.8
  ...
  \item \texttt{runhw1.py} --- \textbf{the driver}. It imports those functions
        ...
        \textbf{This is the file that is run to grade Part F.}
\end{itemize}
...
\noindent So Problem F.6(c) is labelled \texttt{Output of question 6 part c :}\ .
...
which could it not? Name at least one problem from Part B, Part D or Part E and
say precisely what a computer run would and would not establish about it.
\end{prob}
...
E --- dot product, both         & E.1, E.3, E.4 yes, entirely; E.2, E.5, E.6 no. \\
F --- NumPy                     & It \emph{is} the check. \\
\bottomrule
\end{tabular}
