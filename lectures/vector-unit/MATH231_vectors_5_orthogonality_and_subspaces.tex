...
and $\R^{3}$ as lines and planes through the origin; and prove the Pythagorean
identity and use it to account exactly for a projection.
...
\noindent It also extends past two vectors. If $\vc{u}_1,\dots,\vc{u}_k$ are
\emph{mutually} orthogonal, expanding
$\langle\vc{u}_1+\cdots+\vc{u}_k,\ \vc{u}_1+\cdots+\vc{u}_k\rangle$ produces one
square term per vector and one cross term per pair --- and every cross term is
zero. So
\[
  \norm{\vc{u}_1+\cdots+\vc{u}_k}^{2}
  = \norm{\vc{u}_1}^{2}+\cdots+\norm{\vc{u}_k}^{2} .
\]
Lengths do not add, but \emph{squared} lengths do, provided the directions are
mutually orthogonal.
