...
%
%  STATUS: complete.  Sections 1-7.  The unit continues; Sec. 7.4 is the hinge.
%
...
%
%  RULES ADDED BEYOND THE DRAFTED LIST (easy to cut if unwanted; all live in
%  their own table rows in Sec. 2, none is used later in the document):
%      - general exponential  d/dx a^x = a^x ln a
%      - general logarithm    d/dx log_a x = 1/(x ln a)
%      - inverse trig         arcsin, arccos, arctan
%  Everything else in Sec. 2 was on the list.
%
...
as a limit and compute one; say what kind of object a partial derivative is; and
assemble the partials of a function into its gradient.
...
\begin{tabular}{@{}l l l@{}}
\toprule
\textbf{Function} & \textbf{Derivative} & \textbf{Notes} \\
\midrule
$e^{x}$      & $e^{x}$                      & derived in \S4.2--\S4.3 \\
$a^{x}$      & $a^{x}\ln a$                 & $a>0$; reduces to the above when $a=e$ \\
$\ln x$      & $\dfrac{1}{x}$               & $x>0$; derived in \S4.1 \\
$\log_a x$   & $\dfrac{1}{x\ln a}$          & $x>0$, $a>0$, $a\neq1$ \\
\bottomrule
\end{tabular}
\end{center}

\noindent $e^{x}$ is the only function that is its own derivative, which is the
property that makes it the natural base to work in. \S4 derives both of these
...
\begin{tabular}{@{}l l l@{}}
\toprule
\textbf{Function} & \textbf{Derivative} & \textbf{Valid for} \\
\midrule
...
%==============================================================================
The rules in \S2.3 were stated without justification. Here they are earned. The
order matters: the logarithm is done first, from the definition alone, and the
exponential then follows from it.
...
\lookahead Each of those operations turns out to answer a question about $f$.
The direction of $\grad f$, its length, and its dot product against a chosen
unit vector each have a meaning in terms of how $f$ changes --- which is how the
question left open in \S5.3, the rate of change in an \emph{arbitrary}
direction, gets answered from only the $n$ coordinate directions. That is where
this unit goes next.
