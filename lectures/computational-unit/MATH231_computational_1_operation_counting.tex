...
\begin{rmk}[Why that sum is not $O(n^{2})$]
The bound above is not merely convenient --- $n^{3}$ is the right answer, and
$O(n^{2})$ would be false. The sum has a closed form,
\[
  \sum_{k=1}^{n}k^{2} = \frac{n(n+1)(2n+1)}{6} ,
\]
whose leading behaviour is $\tfrac{2n^{3}}{6}=\tfrac{n^{3}}{3}$. If the sum were
$O(n^{2})$ there would be a $C$ with $\tfrac{n^{3}}{3}\lesssim Cn^{2}$ for all
large $n$, which rearranges to $n\lesssim 3C$ --- impossible, since $n$ grows
without bound while $3C$ is fixed. It is worth checking a small case against the
formula: for $n=3$, the sum is $1+4+9=14$ and
$\tfrac{3\cdot4\cdot7}{6}=14$. \checkmark

The moral is that ``$n$ terms, each at most $n^{2}$'' gives $n^{3}$, not
$n^{2}$. Counting the terms is part of the estimate.
\end{rmk}
